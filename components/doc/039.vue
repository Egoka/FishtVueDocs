...
function draw({ circle }: P5I) {
  ...
  const visiblePoints = points.filter(
      (p) => p.y >= offsetY - GLOW_DISTANCE && p.y <= offsetY + h + GLOW_DISTANCE
  );
  ...
  for (const p of visiblePoints) {
    ...
    if (
        Math.abs(nx - mouseX) < mouseCheckRadius &&
        Math.abs(ny - offsetY - mouseY) < mouseCheckRadius
    ) {
      const distToMouse = Math.sqrt(
          (nx - mouseX) ** 2 + (ny - offsetY - mouseY) ** 2
      );
